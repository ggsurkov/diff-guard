...
5. Расширение появится в списке с иконкой; открыть Side Panel — через иконку
   расширения на панели инструментов Chrome (или ПКМ на иконке →
   "Open side panel").

## После изменений в коде

- Изменения в `.svelte`/`.ts` файлах при запущенном `npm run dev`:
  применяются через HMR прямо в открытой side panel, перезагрузка не нужна.
...
- Ошибки service worker смотреть через "Inspect views: service worker" на
  карточке расширения; ошибки side panel — через ПКМ на панели → "Inspect".
